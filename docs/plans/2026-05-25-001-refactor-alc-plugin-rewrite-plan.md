...
type: refactor
created: 2026-05-25
supersedes: docs/plans/2026-05-25-alc-plugin-refactor.md
origin: docs/plans/2026-05-25-alc-plugin-refactor-CONSOLIDATED-REVIEW.md
target_repo: agent-learning-compounder
depth: deep
---

# refactor: ALC plugin — gates-first rewrite with Hermes-DSL apply + agent archive
...
### U10.5. `bin/alc_query.py` — shared read-only query module

**Goal:** Single read-only query API used by MCP server (U17), dashboard server (U10), and CLI subcommands (`alc_apply --list-pending`). Encapsulates SQL against events.sqlite + JSON-reads against patches/, recommendations.json.

**Requirements:** R16 (capability parity — both MCP and dashboard call same functions, can never drift), KTD-13 (apply-log + outcomes are SQL views).

**Dependencies:** U5.5 (events.sqlite), U7 (StateHandle)

**Files:**
- Create: `bin/alc_query.py`
...
- Tests verify each function returns expected shape on fixture sqlite + that read-only mode prevents accidental writes (assertion: attempting INSERT raises OperationalError)
- MCP handlers in U17 become 3-line wrappers (parse args, call alc_query.X, return result). Dashboard's `build_data_blob` becomes a series of alc_query calls.

**Patterns to follow:** KTD-15 named-catalog (each function = documented capability in `references/query-catalog.md` with caller cross-reference)
