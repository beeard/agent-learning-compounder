...
| KTD-16 | Context-boundary enforcement flyttet fra per-artifact-tester til write-path-invariant i event_writer.py + artifact_writer.py. **Reads trust the invariant** (no re-scrub on output); **`EventV4.upgrade_from(v3_row)` re-applies current rules** so v3 backfilled events that violate new rules surface as ValueError (N2). | Én write-path = ett sted å håndheve boundary. Fail-fast ved skrivning, ikke post-hoc-scan. Reads stay flat-cost; v3-upgrade is the only re-enforcement point. Nye event-emittere arvelig safe. U19's boundary-test forenkles fra N artifact-scans til 1 writer-enforcement-test. |
