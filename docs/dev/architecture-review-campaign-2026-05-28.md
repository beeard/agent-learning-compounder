...
| 5 | Runtime Install Target Module | Queued | Next plan/build slice. Move release install target selection behind runtime topology depth while keeping `install.sh` as execution adapter. |
...
The next plan should start at order 5 unless a later architecture review
