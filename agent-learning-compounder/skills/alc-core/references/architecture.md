...
- **Repo bootstrap layer**
  - `../../bin/init_learning_system.py`: creates repo integration, state,
    refresh manifest, and optional hook integration.
- **Ingestion layer**
  - `../../bin/build_repo_baseline.py` and `../../bin/map_active_skills.py` produce
    baseline context and active skill inventories.
  - `../../bin/extract_sessions.py` / `../../bin/distill_learning.py` convert
    session logs into compact reports.
- **Hook layer**
  - `../../bin/collect_hook_event.py` appends normalized hook events.
  - `../../bin/install_runtime_hooks.py` writes runtime adapters after explicit `--apply`.
  - `bin/agent_dispatch.py` owns the shared dispatch field policy used by
    collector, runtime adapters, and MCP reporting.
- **Refresh layer**
  - `../../bin/refresh_learning_state.py` is the intended periodic task, represented
    in a manifest.
