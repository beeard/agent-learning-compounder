...
def render_session_context(
    ...
    *,
    ...
    freshness = freshness or {"source": "alc_init", "mode": "bootstrap-only", "stale": False}
    ...
    lines = [
        ...
        "## Freshness",
        "",
        f"- **Source**: `{freshness.get('source', 'alc_init')}`",
        f"- **Mode**: `{freshness.get('mode', 'bootstrap-only')}`",
    ...
    if workspace_facts:
        ...
        lines.append("")
        for key in ("branch", "dirty_state", "active_plan", "latest_next_action", "latest_report", "latest_validation"):
            if workspace_facts.get(key):
                lines.append(f"- **{key.replace('_', ' ').title()}**: {workspace_facts[key]}")
        lines.append("")
